...
def launch_slurm_job(
    ...
    dependency=None,
    launch_script_name: str = "run_suzuki_4.sh",
    time_limit=None,
) -> int:
    """Launch slurm job
...
python -u {root_dir}/{script_name} {args} > {job_name}.dat"""
    ...
    output = None #subprocess.check_output(f'sbatch {slurm_job_dep} {launch}'.split())
    slurm_jobid = None # int(output.split()[-1])
    os.chdir(top_dir)
    return slurm_jobid
    #return True
...
if __name__ == "__main__":
    ...
    for idx, Nid in enumerate(N):
        ...
        for etaval in eta:
            job_name = f"suzuki_4_{Nid}_{etaval}"
            thc_jobid = launch_slurm_job(
                job_name,
                f"eta_{etaval}",
                root_dir,
                conda_env,
                f"grid_ham_suzuki_4_spectral_norm_exe.py",
