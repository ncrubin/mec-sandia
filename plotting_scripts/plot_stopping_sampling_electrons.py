...
def plot_figures(read_from_file=False):
    outdir = "stopping_sampling_data_el"
    ...
    np.random.seed(7)
    for isamp, num_samples in enumerate([1_00, 1_000, 10_000, 50_000]):
        sim_res = []
        act_res = []
    ...
    plt.savefig("stopping_power_conv_electrons.pdf", bbox_inches="tight", dpi=300)
    ...
    plt.tick_params(which='both', labelsize=14, direction='in')
    plt.savefig("stopping_power_comparison_electrons.pdf", bbox_inches="tight", dpi=300)
